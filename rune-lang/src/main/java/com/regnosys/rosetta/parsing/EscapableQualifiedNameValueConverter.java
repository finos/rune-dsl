 ...
 * given segment converter. A converter given a wildcard additionally accepts it as the last
 * segment, which is what an import ends in; anywhere else it is rejected along with anything else
 * that cannot be written as a Rune identifier.
 */
public class EscapableQualifiedNameValueConverter extends AbstractValueConverter<String> {
	...
	/**
	 * @param segmentConverter converts one segment of the name.
	 * @param separator what separates two segments, from the grammar.
	 * @param wildcard the segment an import may end in, or {@code null} for a rule that allows
	 *            no wildcard.
	 */
	public EscapableQualifiedNameValueConverter(EscapableIDValueConverter segmentConverter, String separator,
			String wildcard) {
	...
	private String mapSegments(String name, UnaryOperator<String> mapper) {
		...
		String[] segments = separatorPattern.split(name, -1);
	...
	private boolean isWildcard(String segment, int index, int segmentCount) {
		// The grammar puts the wildcard after a name, so on its own it is not one.
		return segmentCount > 1 && index == segmentCount - 1 && wildcard != null && wildcard.equals(segment);
	}
}
