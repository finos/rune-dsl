...
public class ModelObjectBuilderGenerator {
	...
	public CodeRenderer builderClass(JavaPojoInterface javaType, JavaPojoBuilderImpl builderImplClass, JavaClassScope scope) {
		...
		return out -> {
			...
			out.indented(() -> {
				...
				out.newline();
				renderMerge(out, properties, builderInterface, extendSuperImpl);
				out.newline();
	...
	private void renderMerge(CodeWriter out, Collection<JavaPojoProperty> properties, JavaPojoBuilderInterface builderType, boolean extended) {
		out.writeln("@SuppressWarnings(\"unchecked\")");
		out.writeln("@Override");
		out.writeln("public ", builderType, " merge(", RosettaModelObjectBuilder.class, " other, ", BuilderMerger.class, " merger) {");
		out.indented(() -> {
			if (extended) {
				out.writeln("super.merge(other, merger);");
			}
			out.writeln(builderType, " o = (", builderType, ") other;");
			out.newline();
			for (JavaPojoProperty prop : properties) {
				if (typeTranslator.isRosettaModelObject(prop.getType())) {
					String getter = prop.getOperationName(GET);
					JavaPojoPropertyOperationType setOperation = typeUtil.isList(prop.getType()) ? GET_OR_CREATE : SET;
					out.writeln("merger.mergeRosetta(", getter, "(), o.", getter, "(), this::", prop.getOperationName(setOperation), ");");
				}
			}
			out.newline();
			for (JavaPojoProperty prop : properties) {
				if (!typeTranslator.isRosettaModelObject(prop.getType())) {
					String getter = prop.getOperationName(GET);
					if (typeUtil.isList(prop.getType())) {
						out.writeln("merger.mergeBasic(", getter, "(), o.", getter, "(), (", Consumer.class, "<", typeUtil.getItemType(prop.getType()), ">) this::", prop.getOperationName(ADD), ");");
					} else {
						out.writeln("merger.mergeBasic(", getter, "(), o.", getter, "(), this::", prop.getOperationName(SET), ");");
					}
				}
			}
			out.writeln("return this;");
