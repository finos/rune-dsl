...
public class PruningTest {
	...
	class OverrideAttributeCardinality extends AbstractPruningTest {
		...
		Stream<Instance> provideTestInstances() {
			return Stream.of(
					Instance.of("Prune optional foo", """
							Parent {
								foo: Foo {
									attr: empty
								}
	...
	private abstract class AbstractPruningTest {
		...
		private Stream<Arguments> provideArguments() {
			return provideTestInstances().map(inst -> 
				Arguments.of(inst.description(), inst.instanceExpression(), inst.expectedPrunedResult()));
		}
