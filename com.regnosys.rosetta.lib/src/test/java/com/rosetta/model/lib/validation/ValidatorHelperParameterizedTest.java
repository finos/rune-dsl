...
public class ValidatorHelperParameterizedTest {

	private static final Function<Foo, ComparisonResult> GREATER_THAN = (foo) -> 
			greaterThan(MapperS.of(foo).map("getAttr1", Foo::getAttr1), MapperS.of(new Integer(5)));
	
	private static final Function<Foo, ComparisonResult> GREATER_THAN_WITH_OR = (foo) -> 
			greaterThan(MapperS.of(foo).map("getAttr1", Foo::getAttr1), MapperS.of(new Integer(5)))
					.or(greaterThan(MapperS.of(foo).map("getAttr2", Foo::getAttr2), MapperS.of(new Integer(5))));
	...
	private static final Function<Bar, ComparisonResult> GREATER_THAN_LIST = (bar) -> 
			greaterThan(MapperS.of(bar).mapC("getFoos1", Bar::getFoos1).map("getAttr1", Foo::getAttr1), MapperS.of(new Integer(5)));
