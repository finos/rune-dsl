...
public class LabelProviderGeneratorTest {
	...
	void testReportLabelOverridesRuleReferenceLabel() throws IOException {
		// On this branch, a rule referenced via [ruleReference ...] can still contribute a label
		// through the legacy `rule ... as "identifier"` syntax. A direct [label ...] annotation on the
		// same attribute overrides it (attr), but where there is none, the rule's label passes through
		// (other) - this legacy path is consulted only for the report-rooted provider, never for the
		// type-rooted one below.
		RosettaTestModel model = loadModel("""
				namespace test
		...
		// Foo carries a direct label on `attr`, so it also gets a type-rooted provider alongside the
		// report-rooted one.
		assertGeneratedFiles(
				"/test/labels/BodyCorpusLabelProvider.java",
				"/test/labels/types/FooLabelProvider.java");
		assertGeneratedFileMatches("report-with-rule-references/BodyCorpusLabelProvider.java", "/test/labels/BodyCorpusLabelProvider.java");
		assertLabels(
			"/test/labels/BodyCorpusLabelProvider.java",
			"attr:My attribute",
			"other:Other from rule"
		);
		// The type-rooted provider never consults rule references: `other` has no label here even
		// though the report-rooted provider above resolves one for it via the legacy rule syntax.
		assertLabels(
			"/test/labels/types/FooLabelProvider.java",
			"attr:My attribute",
			"other:null"
	...
	void testReportLabelOverridesExternalRuleSourceRuleReferenceLabel() throws IOException {
		...
		// Foo carries direct labels (attr, and the path-annotation on bar), so it also gets a
		// type-rooted provider. Bar has none of its own - only rule references - so it gets nothing.
		assertGeneratedFiles(
				"/test/labels/BodyCorpusLabelProvider.java",
				"/test/labels/types/FooLabelProvider.java");
		assertGeneratedFileMatches("report-with-external-source/BodyCorpusLabelProvider.java", "/test/labels/BodyCorpusLabelProvider.java");
		assertLabels(
			"/test/labels/BodyCorpusLabelProvider.java",
			"attr:My attribute",
