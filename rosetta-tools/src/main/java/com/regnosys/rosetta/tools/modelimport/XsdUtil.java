...
//
//		if (xsdName.matches("[a-z]")) {
//			return "lower_" + xsdName;
//		}
//		
    ...
    public void makeNamesUnique(List<? extends RosettaNamed> objects) {
        objects.stream().collect(Collectors.groupingBy(rosettaNamed -> rosettaNamed.getName().toUpperCase()))
                .forEach((name, group) -> {
                    if (group.size() > 1) {
                        for (int i = 0; i < group.size(); i++) {
                            group.get(i).setName(group.get(i).getName() + i);
                        }
                    }
                });
    }
