...
public class XsdElementImport extends AbstractXsdImport<XsdElement, Data> {
    ...
    @Override
    public List<? extends Data> registerTypes(List<XsdAbstractElement> xsdElements, RosettaXsdMapping typeMappings, ImportTargetConfig targetConfig) {
        return super.registerTypes(xsdElements, typeMappings, targetConfig);
    }
    ...
    public Data registerType(XsdElement xsdElement, RosettaXsdMapping typeMappings, ImportTargetConfig targetConfig) {
        ...
        return getXdsData(xsdElement, typeMappings, xsdType, targetConfig);
    }

    private Data getXdsData(XsdElement xsdElement, RosettaXsdMapping typeMappings, XsdNamedElements xsdType, ImportTargetConfig targetConfig) {
        ...
        if (xsdType != null /* TODO */ && typeMappings.hasType(xsdType)) {
            if (xsdType instanceof XsdComplexType) {
                ...
                typeMappings.registerAttribute(xsdElement, valueAttr, data);
    ...
    public void completeType(XsdElement xsdElement, RosettaXsdMapping typeMappings) {
        ...
        if (xsdType instanceof XsdComplexType) {
            ...
            Attribute attr = typeMappings.getAttribute(xsdElement, data);
    ...
    public Map<Data, TypeXMLConfiguration> getXMLConfiguration(XsdElement xsdElement, RosettaXsdMapping xsdMapping, String schemaTargetNamespace) {
        ...
        if (xsdType instanceof XsdComplexType) {
            ...
            if (data.equals(dataType)) {
                typeImport.completeAttributeConfiguration(attributeConfig, xsdType, xsdMapping, result, data);
            }
        } else {
            Attribute attr = xsdMapping.getAttribute(xsdElement, data);
