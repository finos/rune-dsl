...
public class XsdEnumImport extends AbstractXsdImport<XsdSimpleType, RosettaEnumeration> {
    ...
    public RosettaEnumeration registerType(XsdSimpleType xsdType, RosettaXsdMapping typeMappings, ImportTargetConfig targetConfig) {
        ...
        // todo
        List<XsdEnumeration> enumeration = Stream.concat(xsdType.getAllRestrictions().stream(),
                        Optional.ofNullable(xsdType.getUnion()).map(XsdUnion::getUnionElements).stream().flatMap(Collection::stream).flatMap(r -> r.getAllRestrictions().stream()))
                .flatMap(r -> r.getEnumeration().stream())
                .toList();

        Set<String> enumValuesAdded = new HashSet<>();
        enumeration.stream()
                .filter(e -> !e.getValue().isEmpty())
                .filter(x -> enumValuesAdded.add(x.getValue()))
