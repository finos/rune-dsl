			...
			val expected = '''
				...
				public class RepeatableValueRule<INKEY> implements Blueprint<Foo, Integer, INKEY, INKEY> {
					...
					public BlueprintInstance<Foo, Integer, INKEY, INKEY> blueprint() { 
						return 
							startsWith(actionFactory, actionFactory.<Foo, Integer, INKEY>newRosettaRepeatableMapper("__synthetic1.rosetta#//@elements.1/@nodes/@node", "->listAttr", new StringIdentifier("Repeating Value", getClass()), foo -> MapperS.of(foo).<Integer>mapC("getListAttr", _foo -> _foo.getListAttr())))
