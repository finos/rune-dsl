...
public class UnusedElementDiagnosticsProvider implements IWorkspaceDerivedDiagnosticsProvider {
     ...
     *
     * <p>Only five entries differ from what {@link #fallbackNoun} derives: {@code FUNCTION} (so a
     * {@code FunctionDispatch} reads as "Function", not "Function dispatch"), {@code DATA},
     * {@code ROSETTA_SYNONYM_SOURCE} (so its {@code RosettaExternalSynonymSource} subclass reads as "Synonym
     * source", not "External synonym source"), and the two {@code ROSETTA_EXTERNAL_*} ones. The rest are
     * deliberate duplicates of what the fallback derives, so that renaming an {@code EClass} cannot quietly
     * change a user-visible noun.
     */
    private static final List<KindDescriptor> KINDS = List.of(
            ...
            new KindDescriptor(RosettaPackage.Literals.ROSETTA_SYNONYM_SOURCE, "Synonym source"),
