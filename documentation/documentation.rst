...
To be able to specify the ``issuerReference`` attribute as a reference from an existing source, the source must itself implement some cross-referencing mechanism, as follows:

* the object being referenced must be

can be provided from an external cross-reference in the synonym source, using ``href`` as the ``meta`` qualifier.
