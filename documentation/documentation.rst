...
* The ``location`` meta-data qualifier indicates the location of a data attribute within a model instance. This location can be referenced via the ``address`` meta-data qualifier. It is important to note that the location of a data attribute *does not* change with its data value, which is the primary difference from the ``key`` meta-data qualifier.
* The ``address`` meta-data qualifier is used to reference the ``location`` of another data attribute within a model instance. The value of the ``address`` *does not* change with the value of the referenced attribute.
