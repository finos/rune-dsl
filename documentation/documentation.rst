...
* The ``template`` meta-data qualifier indicates that a type is eligible to be used as a data template.  When specified on a type, it is then possible to specify a template reference that cross-references an template object.
