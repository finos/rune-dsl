...
* The ``key`` meta-data qualifier must be added to the type that is being referenced, so that a unique identifier can be associated to objects of that type and make them internally referenceable.
* The ``id`` meta-data qualifier must be added when a basic type attribute is being referenced, so that a unique identifier can be associated to objects of that type and make them internally referenceable.
