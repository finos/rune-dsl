...
**Note**: This example is not part of the Rosetta DSL but corresponds to the default JSON implementation of the model.
...
Thanks to the ``reference`` qualifier, the ``issuerReference`` attribute can simply reference the event issuer party as "Party 2" rather than duplicating its components, through both internal and external references. The internal ``globalReference`` points to the ``globalKey`` hash. The ``externalReference`` points to the "party2" ``externalKey`` sourced from the original FpML document with the implemented ``href`` synonym. Also note that the ``issuerReference`` itself has an associated ``globalKey`` meta-data by default since its ``Identifier`` class has a ``key`` qualifier.
