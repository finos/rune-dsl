...
A model's `@model` name and its default serialisation format are both configured in that model's `rune-config.yml`, which must live at `src/main/resources` in the model project. For example, the demo model's [rune-config.yml](https://github.com/rosetta-models/demo/blob/master/rosetta-source/src/main/resources/rune-config.yml):
...
| New | Example | Desc |
| --- | --- | --- |
| `@model` | `"@model": "DEMO"` | This is the short name for the model, as configured in `rune-config.yml` (see above). |
