...
### 1.3 `calculation`
The `calculation` annotation marks a function as a fully implemented calculation.
...
func FixedAmount:
  ...
  set fixedAmount: notional * rate
```

There is also a built-in `calculation` type alias for `string`, which can be used as an attribute type to indicate that the value represents the output of a calculation.
...
### 1.5 Using metadata in functions and expressions
