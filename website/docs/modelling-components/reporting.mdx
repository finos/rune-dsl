...
### 2.5 Labels beyond reporting
`label` annotations are not specific to reports — any type in a Rune model can carry them on its own, inherited, or overridden attributes, and Rune generates a label provider for that type regardless of whether it is used as a report or function output. This is what lets labelled serialisation (for example, CSV) match columns by label for any type, not only report types. See [Label providers](/docs/developers/rune-and-java#13-label-providers) for how this is generated and exposed in Java.
...
`docReference` links the rule to the exact regulatory text that justifies it, as outlined in the document reference section of the [Metadata component](./metadata#2-document-reference).
...
reporting rule EuroEmissionStandard from ReportableEvent:
  [docReference EuropeanCommission StandardEmissionsEuro6 article "1"
...
reporting rule NotionalAmountScheduleLeg1 from ReportableEvent: <"Notional Amount Schedule">
	[docReference CFTC Part45 appendix "1" dataElement "33-35" field "Notional Amount Schedule"
...
reporting rule NotionalAmountScheduleLeg1EffectiveDate from DatedValue: <"Effective date of the notional amount of leg 1">
  [docReference CFTC Part45 appendix "1" dataElement "34" field "Effective date of the notional amount of leg 1"
