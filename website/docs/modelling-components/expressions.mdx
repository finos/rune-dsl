...
### 2.3 Narrowing to a subtype with `as`
Use the `as` operator to narrow a value to one of its subtypes:
`<expression> as <Type>`

It can be used in two situations:
- On a **choice type**, where the right-hand side may be any (nested) option of the choice. For a choice type `Payout` with an option `PerformancePayout`, you can access that option directly:
  ```haskell
  payout as PerformancePayout
  ```
  If the payout is not a PerformancePayout, then the result is empty.
- On a **data type**, where the right-hand side may be any (transitive) extension of that data type.

**Semantics:** `as` filters its input by type.
- For a single-cardinality input, the result is the input if it is of the given type, or empty otherwise.
- For a multi-cardinality input, the result is the list of all items that are of the given type.

```haskell
// Returns the payout if it is an `OptionPayout`, or empty otherwise.
payout as OptionPayout

// Keeps only the `OptionPayout` items of a list of payouts.
payouts as OptionPayout
```
...
5. Unary operators: `>, ->>, as, exists, is absent, only-element, flatten, distinct, reverse, first, last, sum, one-of, choice, to-string, to-number, to-int, to-time, to-enum, to-date, to-date-time, to-zoned-date-time, switch, sort, min, max, reduce, filter, map, extract, with-meta`
...
This works for any name, including each segment of a namespace, of an import and of a fully qualified reference:

```haskell
namespace ^type.vehicle

import ^enum.tax.*

type Vehicle:
    band ^enum.tax.VehicleTaxBandEnum (1..1)
```
