...
The `suppressWarnings` annotation is used to suppress warnings generated by the Rune DSL validators.
The category of warnings to suppress can be specified using the `category` attribute. Currently Rune only supports
one category of warning suppression which is `capitalisation`. 

An example of how to suppress capitalisation warnings on a type is shown below. In this example you will not get a warning telling you to use a capital letter for the start of the type name:

```Haskell
type myType:
  [suppressWarnings capitalisation]
  myAttribute string (1..1)
```

To suppress warnings on an attribute you can place the `suppressWarnings` annotation on the attribute itself. In this example you will not get a warning telling you to use a lower case letter for the start of the attribute name:
...
You can suppress the capitalisation warnings on all attributes and the type name by putting the annotation on the type as follows:

```Haskell
type myType:
  [suppressWarnings capitalisation]
    MyAttribute string (1..1)
    myOtherAttribute string (1..1)
```

Capitalisation warnings can also be suppressed on enumerations names, conditions names and functions names by placing the annotation directly on the object to suppress, as follows:
